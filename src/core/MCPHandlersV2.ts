...
toolRegistry.register('web_search', {
  ...
  async execute(input: any) {
    // 模拟实现 - 实际项目中替换为真实的 web search
    return {
      results: [
        { title: 'Sample Result 1', url: 'https://example.com/1', snippet: 'Sample search result 1' },
        { title: 'Sample Result 2', url: 'https://example.com/2', snippet: 'Sample search result 2' },
      ],
    };
  },
});
...
toolRegistry.register('web_fetch', {
  ...
  async execute(input: any) {
    // 模拟实现 - 实际项目中替换为真实的 web fetch
    return {
      content: `Fetched content from ${input.url}`,
...
toolRegistry.register('file_glob', {
  ...
  async execute(input: any) {
    ...
    const safePath = path.resolve(basePath);
    const workingDir = process.cwd();
  ...
  async execute(input: any) {
    ...
    try {
      ...
      return new Promise((resolve) => {
        const grep = spawn('grep', ['-r', input.pattern, safePath], {
          encoding: 'utf-8',
        });
